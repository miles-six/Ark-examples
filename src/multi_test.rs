...
use blake2::digest::{XofReader, generic_array::typenum::Xor};
...
impl<F:Field> ConstraintSynthesizer<F> for EquationCircuit<F>{
    ...
    let out_value = x_sqr_value.map(|mut e|{
...
mod test {
    ...
    fn test_groth16_circuit_multiply() {
        ...
        assert!(!Groth16::<Bls12_377>::verify(&vk, &[a], &proof).unwrap());
    }
    #[test]
